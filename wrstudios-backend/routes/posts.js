...
async function attachTags(posts) {
  ...
  const [rows] = await db.query(
    `SELECT pt.post_id, t.tag_name
     ...
     WHERE pt.post_id IN (?) ORDER BY pt.post_id, pt.position`,
    [ids]
  );
  const byPost = new Map(ids.map(id => [id, []]));
  for (const row of rows) byPost.get(row.post_id)?.push(row.tag_name);
  return posts.map(post => ({ ...post, tags: byPost.get(post.post_id) || [] }));
}
...
async function saveTags(postId, tags) {
  for (const [position, tag] of tags.entries()) {
    ...
    await db.query(
      'INSERT IGNORE INTO post_tags (post_id, tag_id, position) VALUES (?, ?, ?)',
      [postId, result.insertId, position]
    );
  }
}
...
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    if (tags.length) {
      try {
        await saveTags(post_id, tags);
      } catch (tagError) {
        // Tin vẫn đăng được nếu việc lưu tag gặp lỗi.
        console.warn('Không lưu được tag của tin:', tagError.message);
      }
    }
